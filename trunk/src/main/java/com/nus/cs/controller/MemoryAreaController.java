 ...
 */
@Controller
public class MemoryAreaController {
	...
	public String ma(Locale locale, Model model) {
		...
		DBTO dbTO = null;
		try {
			dbTO = dbService.getData(Constants.MEMORY_AREA_ID, DateUtil.getDateAsString(before),
	...
	@RequestMapping(value = "/ma2", method = RequestMethod.POST)
	public String ma2(Model model,
		...
		try {
			dbTOList = dbService.getDataList(Constants.MEMORY_AREA_ID, startTime, endTime, x);
		} catch (InstantiationException e) {
			e.printStackTrace();
		} catch (IllegalAccessException e) {
			e.printStackTrace();
	...
	@RequestMapping(value = "/ma3", method = RequestMethod.POST)
	public String ma3(Model model, 
		...
		List<DBTO> dbTOList = null;
		try {
			dbTOList = dbService.getDataList(Constants.MEMORY_AREA_ID, startTime, endTime, y);
		} catch (InstantiationException e) {
			e.printStackTrace();
		} catch (IllegalAccessException e) {
			e.printStackTrace();
