					...
					<td valign="middle" style="padding: 30px; width: 100%;">
						<table border="0" width="100%">
							<tr align="center" bgcolor="#1B1B1B">
								...
								<th><font color="#F2F2F2">Shared Pool Free %</font></th>
							...
							<%
								int i = 0;
							%>
							<c:forEach items="${spTOList}" var="spTO" varStatus="status">
								<%
									i++;
								%>
								<%
								...
								<tr align="center" bgcolor="#F2F2F2">
									...
									<td><h5><%=i%></h5></td>
									<td><h5>
											<c:out value="${spTOList[status.index].startTime}" />
										</h5></td>
									<td><h5>
											<c:out value="${spTOList[status.index].endTime}" />
										</h5></td>

									<c:choose>
										<c:when test="${spTOList[status.index].status == HEALTHY}">
											<td style="color: #00FF00">
										</c:when>
										<c:when test="${spTOList[status.index].status == MODERATE}">
									...
									<h5>
										<fmt:formatNumber type="percent" minFractionDigits="2"
											maxFractionDigits="2"
											value="${spTOList[status.index].avgValue/100}" />
									</h5>
									</td>

									<c:choose>
										<c:when test="${bcTOList[status.index].status == HEALTHY}">
											<td style="color: #00FF00">
										</c:when>
										<c:when test="${bcTOList[status.index].status == MODERATE}">
											<td style="color: #FFBF00">
										</c:when>
										<c:otherwise>
											<td style="color: #DF0101">
									...
									<h5>
										<fmt:formatNumber type="percent" minFractionDigits="2"
											maxFractionDigits="2"
											value="${bcTOList[status.index].avgValue/100}" />
									</h5>
									</td>

									<c:choose>
										<c:when test="${rlbTOList[status.index].status == HEALTHY}">
											<td style="color: #00FF00">
										</c:when>
										<c:when test="${rlbTOList[status.index].status == MODERATE}">
											<td style="color: #FFBF00">
										</c:when>
										<c:otherwise>
											<td style="color: #DF0101">
									...
									<h5>
										<fmt:formatNumber type="percent" minFractionDigits="2"
											maxFractionDigits="2"
											value="${rlbTOList[status.index].avgValue/100}" />
									</h5>
									</td>

									<c:choose>
										<c:when test="${rlfTOList[status.index].status == HEALTHY}">
											<td style="color: #00FF00">
										</c:when>
										<c:when test="${rlfTOList[status.index].status == MODERATE}">
											<td style="color: #FFBF00">
										</c:when>
										<c:otherwise>
											<td style="color: #DF0101">
									...
									<h5>
										<fmt:formatNumber type="number" minFractionDigits="3"
											maxFractionDigits="3"
											value="${rlfTOList[status.index].avgValue}" />
									</h5>
									</td>

									<c:choose>
										<c:when test="${maTOList[status.index].status == HEALTHY}">
											<td style="color: #00FF00">
										</c:when>
										<c:when test="${maTOList[status.index].status == MODERATE}">
											<td style="color: #FFBF00">
										</c:when>
										<c:otherwise>
											<td style="color: #DF0101">
									...
									<h5>
										<fmt:formatNumber type="percent" minFractionDigits="2"
											maxFractionDigits="2"
											value="${maTOList[status.index].avgValue/100}" />
									</h5>
									</td>
