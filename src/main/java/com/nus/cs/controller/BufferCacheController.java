...
public class BufferCacheController {
	...
	public String sp(Locale locale, Model model) {
		...
		DBTO dbTO = null;
		try {
			dbTO = dbService.getData(Constants.BUFFER_CACHE_ID, DateUtil.getDateAsString(before),
	...
	public String sp2(Model model,
		...
		try {
			dbTOList = dbService.getDataList(Constants.BUFFER_CACHE_ID, startTime, endTime, x);
		} catch (InstantiationException e) {
			e.printStackTrace();
		} catch (IllegalAccessException e) {
			e.printStackTrace();
	...
	public String sp3(Model model, 
		...
		List<DBTO> dbTOList = null;
		try {
			dbTOList = dbService.getDataList(Constants.BUFFER_CACHE_ID, startTime, endTime, y);
		} catch (InstantiationException e) {
			e.printStackTrace();
		} catch (IllegalAccessException e) {
			e.printStackTrace();
